...
namespace LoginAutenticacao.Dominio.Usuarios.Servicos
{
    public class UsuariosServico : IUsuariosServico
    {
        ...
        public Usuario Instanciar(string nome, string email, string login, string senha)
        {
            return new(nome, email, login, senha);
        }
        ...
        public void Inserir(Usuario usuario)
        {
            ...
            usuarioExistente = usuariosRepositorio.Query().FirstOrDefault(x => x.Login.Equals(usuario.Login));
            if (usuarioExistente is not null)
            {
                throw new RegraDeNegocioExcecao("Login inválido");
            }
